...
def check_web_config_consistency(config):
    ...
    for k, v in list(login_conf_deps.items()):
        if not config.has_option('login', k):
            for needed in v:
                if not config.has_option('login', needed):
...
def console_script(fsroot=''):
    ...
    config_file = os.path.join(fsroot + args.config).replace("//", "/")
