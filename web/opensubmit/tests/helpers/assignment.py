...
def create_validated_assignment(course, grading_scheme, casedir, validation_script, fulltest_script=None):
    ...
    relative_temp_dir = "./reltmp/"
    os.makedirs(relative_temp_dir, exist_ok=True)

    # Move test files to current MEDIA_ROOT,
    # otherwise Django security complains
    os.system('cp {0}/* {1}'.format(rootdir + casedir, relative_temp_dir))

    vs = open(relative_temp_dir + validation_script, 'rb')
    if fulltest_script:
        fs = open(relative_temp_dir + fulltest_script, 'rb')
    else:
        fs = None
