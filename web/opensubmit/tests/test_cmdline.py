...
class CmdLine(TestCase):
    ...
    def set_cfg(self, section: str, option: str, value: str):
    ...
    def setUp(self):
        ...
        # for this test the user will be the running user, because not all systems allow non root chown
        self.old_cmdline_uid_functions = (cmdline.webserver_uid, cmdline.webserver_gid)
        cmdline.webserver_uid, cmdline.webserver_gid = os.getuid, os.getgid
        ...
        self.set_cfg('server', 'HOST', 'http://www.troeger.eu')
        self.set_cfg('server', 'MEDIA_ROOT', self.tmpdir + settings.MEDIA_ROOT)
        self.set_cfg('server', 'LOG_FILE', self.tmpdir + settings.LOG_FILE)
        self.set_cfg('database', 'DATABASE_NAME', self.tmpdir + 'database.sqlite')
        self.set_cfg('login', 'LOGIN_GOOGLE_OAUTH_KEY', 'foo')
        self.set_cfg('login', 'LOGIN_GOOGLE_OAUTH_SECRET', 'bar')
    ...
    def tearDown(self):
        # repair the cmdline class
        cmdline.webserver_uid, cmdline.webserver_gid = self.old_cmdline_uid_functions
    ...
    def test_bool_configcreate_env(self):
        ...
        cmdline.console_script(fsroot=self.tmpdir)
        with open(self.conf_name, 'rt') as file:
            data = file.readlines()
